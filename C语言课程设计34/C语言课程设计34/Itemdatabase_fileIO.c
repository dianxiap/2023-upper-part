...
const char* item_name_str = "name_";
...
/*成功则返回1，否则返回0*/
{
    FILE* fp;
    char buffer[1024];
    size_t file_version_length = strlen(file_version);
    ...
    /*读入一行并检查是否与文件版本匹配*/
    ReadLineFromFile(fp, buffer, sizeof(buffer) - 1);
    if (strncmp(buffer, file_version, file_version_length) != 0)
    {
        fclose(fp);
        return 0;
    }
    if (strcmp(buffer + file_version_length, "10") != 0)
    {
        fclose(fp);
        return 0;
    }
    ...
    while (!feof(fp))
    {
        ...
        if (in_item_flag)
        {
            ...
            {
                strncpy(temp_item.name, buffer + item_name_str_len, ITEM_NAME_MAX_LEN);
                temp_item.name[ITEM_NAME_MAX_LEN] = '\0';
                continue;
            }

            /*检查当前行是否是物品编号*/
            if (strncmp(buffer, item_number_str, item_number_str_len) == 0)
            {
                sscanf(buffer + item_number_str_len, "%d", &temp_item.number);
                continue;
            }

            /*检查当前行是否是物品数量*/
            if (strncmp(buffer, item_quantity_str, item_quantity_str_len) == 0)
            {
                sscanf(buffer + item_quantity_str_len, "%d", &temp_item.quantity);
                continue;
            }
        }
        else
